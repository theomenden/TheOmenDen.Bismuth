...
public class DefaultColumns {
    ...
    private static final Object2ObjectLinkedOpenHashMap<ResourceLocation, ResourceLocation> dynamicColumns= new Object2ObjectLinkedOpenHashMap<>();
    ...
    public static void reloadDefaultColumnBoundaries(RegistryAccess manager) {
        ...
        if(manager != null) {
            ...
            biomeRegistry
                    ...
                    .forEach(key -> dynamicColumns.putIfAbsent(key.location(), computeClosestDefaultBiomeByRegistry(key, biomeRegistry)));
        }
    }
    ...
    public static ColumnBounds getLegacyBoundaries(ResourceKey<Biome> biomeKey, Registry<Biome> biomeRegistry, boolean isUsingOptifine) {
        var bounds = SimpleBiomeRegistryUtils.getLegacyColumns().get(biomeKey.location());
       if(bounds == null) {
           if(isUsingOptifine) {
               ...
               bounds = SimpleBiomeRegistryUtils.getLegacyColumns().get(vanillaBiomeApproximation(biomeKey));
               if(bounds == null) {
    ...
    public static ColumnBounds getStableBoundaries(ResourceKey<Biome> biomeResourceKey) {
        var bounds = SimpleBiomeRegistryUtils.getStableColumns().get(biomeResourceKey.location());

        if(bounds == null) {
            bounds = SimpleBiomeRegistryUtils.getStableColumns().get(vanillaBiomeApproximation(biomeResourceKey));
    ...
    private static Object2ObjectLinkedOpenHashMap<ResourceLocation, ColumnBounds> createStableColumnBoundaries() {
        ...
        return new Object2ObjectLinkedOpenHashMap<>(stableColumnResults);
    }
}
