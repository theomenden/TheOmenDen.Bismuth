...
import com.theomenden.bismuth.client.BismuthConfig;
...
public abstract class LightTextureMixin {
    ...
    @Shadow protected abstract float getDarknessGamma(float delta);

    @Shadow protected abstract float calculateDarknessScale(LivingEntity entity, float factor, float delta);
    ...
    private void onLightTextureUpdate(float partialTicks, CallbackInfo ci) {
        ...
        relativeIntensityExponent = ambience * BismuthConfig.calculateScale(Bismuth.configuration.relativeBlockLightIntensity) * MathUtils.INV_16;

        if(level.getSkyFlashTime() > 0 ) {
            ambience = -1.0f;
        }

        LightMapping map = LightMappings.getLightMapping(level);
